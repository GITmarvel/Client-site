...
document.addEventListener("DOMContentLoaded", () => {
  gsap.registerPlugin(ScrollTrigger);
  ScrollTrigger.normalizeScroll(true); // smoother + more stable scrolling

  // -----------------------------------
  // ⭐ MOBILE-OPTIMIZED LENIS (slower)
  // -----------------------------------
  const isMobile = window.matchMedia("(max-width: 768px)").matches;

  const lenis = new Lenis({
    duration: isMobile ? 1.4 : 1.1,          // smoother easing
    wheelMultiplier: isMobile ? 0.55 : 0.8,  // slower scroll
    touchMultiplier: isMobile ? 0.5 : 0.9,   // softer touch scrolling
    smoothWheel: true,
    smoothTouch: true,
  });
